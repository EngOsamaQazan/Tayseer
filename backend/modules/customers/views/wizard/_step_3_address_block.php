 ...
 *
 * Collapsible variant:
 *   When $collapsible is true the block becomes a native <details> with
 *   a sticky <summary> header. The map widget only initializes after the
 *   block is expanded for the first time (Leaflet doesn't measure tile
 *   sizes correctly inside display:none containers). The block opens by
 *   default if the saved draft already contains data — no information is
 *   ever hidden by surprise.
 *
 * @var string $key          'home' | 'work' — used for prefix + scoping
 * @var string $title        Section title (Arabic)
 * @var string $iconCss      FontAwesome CSS class (e.g. 'fa-home')
 * @var int    $typeCode     1 = work, 2 = home (matches Address.address_type)
 ...
 * @var bool   $collapsible  When true, render as a native <details> shell
 */

$key         = $key         ?? 'home';
$title       = $title       ?? 'العنوان';
$iconCss     = $iconCss     ?? 'fa-map-marker';
...
// Detect whether the draft has *any* meaningful content. Drives both the
// initial open-state (collapsible blocks auto-open when there's data so
// users never lose sight of saved input) and the summary chip.
$hasAnyValue = false;
foreach (['address_city','address_area','address_street','address_building',
          'postal_code','address','latitude','longitude','plus_code'] as $k) {
    if (trim((string)($values[$k] ?? '')) !== '') { $hasAnyValue = true; break; }
}

// Build the summary "chip" text shown next to the title in the collapsed
// header. Mirrors the JS `refreshChip()` logic so the first paint matches
// what later edits will produce — no flicker on toggle.
$chipParts = [];
...
<details class="cw-fieldset cw-fieldset--collapsible cw-addr-block"
    ...
    <summary class="cw-fieldset__summary">
        <span class="cw-fieldset__title">
            <i class="fa <?= Html::encode($iconCss) ?>" aria-hidden="true"></i>
            <?= Html::encode($title) ?>
        </span>
    ...
    <div class="cw-addr-block__body">
        ...
        <?php endif ?>
<?php else: ?>
<div class="cw-fieldset cw-addr-block" data-cw-addr-block="<?= Html::encode($key) ?>">
...
<?php endif ?>
    ...
    <input type="hidden" name="<?= Html::encode($prefix) ?>[address_type]" value="<?= (int)$typeCode ?>">
